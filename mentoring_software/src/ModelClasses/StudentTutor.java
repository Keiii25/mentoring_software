...
public class StudentTutor extends User implements StudentActions, TutorActions, GetContracts, GetBid, GetBids, CloseDownBid, MakeContract, MakeMessage {
    // constructor
    public StudentTutor(String givenName, String familyName, String id, String userName, JSONArray competencies, JSONArray qualifications, View view){
        super(givenName, familyName, id, userName, true, true, competencies, qualifications, view);
    ...
    public void updateActionUI() {
        // ModelClasses.Student Side
        // getting a JSONArray of user's contracts
        ...
        aboutToExpireContracts.putAll(jobContractsAndItsBidIds.getJSONArray("aboutToExpireContracts"));
        ...
        // getting student's action
        ArrayList<Action> studentTutorActions = getStudentActions(view, this, myContracts, myActiveBids, expiredContracts);

        // getting tutor's actions
        ArrayList<Action> tutorActions = getTutorActions(view, availableBids, jobContracts, offerContracts, this);

        // adding tutor's action into studentTutor's action
        for (int i = 0; i < tutorActions.size(); i++) {
            studentTutorActions.add(tutorActions.get(i));
        }

        // getting default user actions and adding them to student tutor's actions
        ArrayList<Action> defaultActions = getDefaultActions();

        for (int i =0; i < defaultActions.size(); i++) {
            studentTutorActions.add(defaultActions.get(i));
        }

        // setting studentTutor actions
        super.setActions(studentTutorActions);
    }
}
